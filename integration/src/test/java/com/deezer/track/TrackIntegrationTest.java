...
class TrackIntegrationTest {
    ...
    @BeforeEach
    void setupClass() {
        RestTemplate shazamRestTemplate = new RestTemplateBuilder()
                .rootUri(String.format("http://localhost:%d", server.getPort()))
                .build();

        trackIntegration = new TrackIntegration(shazamRestTemplate);
    }
    ...
    void whenFindTrackReturnArtistIntegration() throws JsonProcessingException {
        ...
        var actual = trackIntegration.findTrack("54428397");
